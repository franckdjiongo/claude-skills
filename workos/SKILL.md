...
| Verify AuthKit tokens in a custom backend (Convex `auth.config.ts`, any JWKS verifier) | `references/workos-authkit-jwt-custom-backend.md` |
| Debug a login loop / CORS on `user_management/authenticate` / "No auth provider found" / 403 after successful sign-in | `references/workos-authkit-jwt-custom-backend.md` |
