#!/usr/bin/env python3
# ANALYZER_VERSION = "v1.31 — additive: Codex (OpenAI) transcript support. Codex Desktop exports a different JSONL schema ({timestamp, type, payload} with type in session_meta/event_msg/response_item, model_provider=openai) that the Claude event parser could not read: analyze() returned scope_tier=Micro with 0 tools and None duration for real, large sessions — a DANGEROUSLY misleading skeleton (observed 2026-08-24, banc d'essai cloud: 4 of 6 exported transcripts were Codex and each rendered as an empty Micro session, so the reviewer had to hand-read every raw JSONL). New: is_codex_transcript() detects the format from a small line prefix; analyze_codex() produces a report shaped like the Claude one (session{}, scope_tier, tool_usage, event_counts, turns, user_turns_detail via the SHARED classify_user_turns) plus source_format='codex', format_supported=False, and degradation_notes[]. Codex tool calls are embedded as '[external_agent_tool_call: <Tool>' markers inside agent_message text (not separate items) — _codex_extract_tool_calls() recovers real per-tool counts by regex, so scope_tier is computed from genuine signal, not mislabeled Micro. render_markdown() dispatches Codex reports to render_codex_markdown(). The Claude path (detection returns False) is byte-for-byte unchanged; every existing key still emitted for Claude transcripts; CLI flags unchanged. Purely additive: a format that produced a useless report now produces a degraded-but-honest one. 2026-08-24"
...
# ─── v1.31 additive: Codex (OpenAI) transcript support ──────────────────────


def is_codex_transcript(events):
    """v1.31: True if this JSONL is a Codex Desktop export rather than a Claude
    Code transcript.

    Codex lines carry a top-level ``payload`` and a ``type`` in
    {session_meta, event_msg, response_item}; Claude Code lines never carry a
    top-level ``payload``. Scan a small prefix so a single stray line cannot
    flip the verdict, and require at least one Codex-typed line.
    """
    for ev in events[:25]:
        if (
            isinstance(ev, dict)
            and "payload" in ev
            and ev.get("type") in ("session_meta", "event_msg", "response_item")
        ):
            return True
    return False


# v1.31: Codex embeds tool calls inline in assistant message text as
# "[external_agent_tool_call: <ToolName> ...]" markers (they are NOT separate
# response_item records the way Claude tool_use blocks are). Recover the tool
# name that follows the marker so scope_tier rests on genuine tool signal.
_CODEX_TOOL_CALL_RE = re.compile(r"\[external_agent_tool_call:\s*([A-Za-z_][\w]*)")


def _codex_text_of(payload):
    """Return the text carried by a Codex event/response payload, or ''."""
    if not isinstance(payload, dict):
        return ""
    # event_msg/agent_message and event_msg/user_message carry a flat "message".
    msg = payload.get("message")
    if isinstance(msg, str):
        return msg
    # response_item/message carries content[] blocks with input_text/output_text.
    out = []
...
def _codex_extract_tool_calls(text):
    """Yield tool names from the inline [external_agent_tool_call: X] markers."""
    for m in _CODEX_TOOL_CALL_RE.finditer(text or ""):
        yield m.group(1)


def analyze_codex(events, jsonl_path):
    """v1.31: degraded-but-honest report for a Codex transcript.

    Shaped like the Claude report (session{}, scope_tier, tool_usage,
    tool_usage_total, event_counts, turns, user_turns_detail) so existing
    consumers keep working, plus source_format/format_supported/
    degradation_notes to flag what could not be measured. Tool telemetry is
    reconstructed from inline markers; anything Codex does not export
    (native subagent fan-out cost, git branch, per-tool inputs) is left absent
    and named in degradation_notes rather than guessed.
    """
    session_meta = {}
    first_ts = None
    last_ts = None
    ...
    # user turns are built in the SAME dict shape classify_user_turns() expects,
    # so the shared classifier (and every v1.25–v1.30 category) applies unchanged.
    user_turns = []
    # Codex has no tool_use events between user turns; use assistant-message
    # activity since the previous user turn as the autonomy proxy that the
    # classifier reads from `tool_uses_since_prev_user`.
    activity_since_prev_user = 0
    ...
    for ev in events:
        ...
        event_counts[pl.get("type") or t] += 1

        ts_parsed = iso_from_timestamp(ev.get("timestamp"))
        if ts_parsed:
        ...
        pltype = pl.get("type")
        text = _codex_text_of(pl)

        # Tool calls are inline in assistant text; count them wherever they appear.
        if pltype in ("agent_message", "message"):
            role = pl.get("role")
            if pltype == "agent_message" or role == "assistant":
                for name in _codex_extract_tool_calls(text):
                    tool_usage[name] += 1

        # Token usage: keep the last non-null total.
        if pltype == "token_count":
            info = pl.get("info") or {}
            tot = (info.get("total_token_usage") or {}).get("total_tokens")
            if isinstance(tot, int):
                token_total = tot

        # User turns come from event_msg/user_message (the human's turns).
        if pltype == "user_message" and isinstance(text, str) and text.strip():
            stripped = text.strip()
            is_command = "<command-name>" in stripped or stripped.startswith("/")
            is_harness = (
                "<task-notification>" in stripped
            ...
            preview = (cleaned[:250] if cleaned else stripped[:250]).replace("\n", " ")
            user_turns.append({
                "ordinal": len(user_turns) + 1,
                "length": len(stripped),
                "tool_uses_since_prev_user": activity_since_prev_user,
                "is_command_invocation": is_command,
                "harness_injected": is_harness,
                "preview": preview,
            })
            activity_since_prev_user = 0
        elif pltype in ("agent_message", "message"):
            activity_since_prev_user += 1

    tool_total = sum(tool_usage.values())
    n_agent = tool_usage.get("Agent", 0) + tool_usage.get("Task", 0)
    n_user_nontrivial = sum(
        1 for u in user_turns if not u["harness_injected"] and u["length"] > 40
    )

    scope_tier = classify_scope_tier({
        "tool_usage": dict(tool_usage),
        "subagents_count_total": n_agent,
        "user_turns_nontrivial": n_user_nontrivial,
    })

    turns_detail = classify_user_turns(user_turns)
    ...
    degradation_notes = [
        "Codex (OpenAI) transcript: the Claude event parser does not apply; "
        "this is a degraded extraction (source_format='codex', "
        "format_supported=False).",
        "Tool calls were reconstructed from inline "
        "'[external_agent_tool_call: <Tool>]' markers in assistant text; "
        "per-tool INPUTS, tool_result outcomes, and native subagent fan-out "
        "cost are not exported by Codex and are absent here.",
        "git branch, forked_from, and Claude-specific harness events "
        "(compaction, Stop-hook activations) are not present in Codex exports.",
    ]
    if all(u["tool_uses_since_prev_user"] == 0 for u in user_turns) and tool_total == 0:
        degradation_notes.append(
            "No inline tool markers found — assistant activity is measured only "
            "by message count; scope_tier may under-read the real work."
        )
    ...
    return {
        ...
        "format_supported": False,
        "session": {
            "id": session_meta.get("session_id") or session_meta.get("id"),
        ...
        "scope_tier_basis": (
            "computed from inline-marker tool counts + user turns (Codex-degraded)"
        ),
        "event_counts": dict(event_counts),
        "turns": {
            "user_total_events": event_counts.get("user_message", 0),
            "user_nontrivial": n_user_nontrivial,
            "assistant_messages": event_counts.get("agent_message", 0),
        },
        "tool_usage": dict(tool_usage),
        "tool_usage_total": tool_total,
        "subagents": {"by_type": {}, "by_description": {}, "count_total": n_agent},
        "skills_invoked": {},
        "user_turns_detail": turns_detail,
        "failure_patterns": {},
...
def analyze(jsonl_path: Path):
    ...
    # v1.31 additive: Codex transcripts use a different schema the Claude event
    # parser cannot read (it would yield an empty Micro report). Detect and route
    # to the dedicated degraded extractor. Claude transcripts fall straight
    # through — this branch is never taken for them.
...
def render_codex_markdown(report):
    """v1.31: markdown for a Codex (degraded) report."""
    s = report["session"]
    lines = [
        "## Session signals (deterministic — ⚠️ Codex, degraded)",
        "",
        "> **Source format: Codex (OpenAI).** The Claude analyzer cannot parse "
        "native tool telemetry from this export — the counts below are "
        "reconstructed from inline markers and the tier is a proxy. Read the raw "
        "transcript for anything the fields do not carry.",
        "",
        f"- **Session ID:** `{s.get('id')}`",
        f"- **Project cwd:** `{s.get('cwd')}`",
        f"- **Originator / provider:** {s.get('originator')} / {s.get('model_provider')}",
        f"- **Duration:** {s.get('duration_minutes_rounded')} min "
        f"({s.get('duration_seconds')}s)",
        f"- **Scope tier (proxy):** **{report.get('scope_tier')}** "
        f"({report.get('scope_tier_basis')})",
        "",
        "### Tool usage (from inline `[external_agent_tool_call]` markers, "
        f"total: {report.get('tool_usage_total', 0)})",
    ]
    for name, n in sorted(
        report.get("tool_usage", {}).items(), key=lambda kv: -kv[1]
    ):
        lines.append(f"- `{name}`: {n}")
    tk = report.get("token_usage_total")
    if tk is not None:
        lines += ["", f"### Token usage (reported total): {tk}"]
...
def render_markdown(report):
    ...
    # v1.31 additive: Codex reports have their own (degraded) renderer.
