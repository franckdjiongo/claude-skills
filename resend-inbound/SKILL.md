...
See `resend-send-email` skill's [installation guide](../resend-send-email/references/installation.md) for full installation commands.
...
If you set up Resend to receive email on a root domain, *all* traffic will be routed to Resend, not to any other mailbox. Use a subdomain with inbound emails.
