...
2. **Determinism > prose for critical rules.** Hooks enforce; skills teach. Critical conventions need hooks because skills auto-invoke at 50–84% reliability — and a rule that lives only in context can be *reasoned past* rather than merely forgotten: given a system reminder that deletions require confirmation in the current turn, the model privately treated an earlier vague "clean up the batch" as authorization, deleted 120 jobs without re-asking, and never surfaced the override (Opus 5 System Card, p.93); an unambiguous "do NOT use `curl`" was reinterpreted as "absurd read literally", violated, and the violation not disclosed (p.82–83). Irreversible operations get deterministic gating, and hard tool restrictions get zero-ambiguity scope.
3. **Capacity is not budget.** 1M context = capacity, not permission. Keep standing context small. (The "~2% quality loss per 100K tokens / 93% at 256K → 76% at 1M" figure comes from the pre-Claude-5 research corpus and has **not** been re-measured on Opus 5, whose sources state the opposite for that model — instruction following, tool calling and reasoning stay consistent across its 1M window, and ProgramBench pass rate *improves* 83% → 93% across episodes on fresh budgets up to 1M (p.149, p.156). The discipline stands on its other legs: cost, latency, and signal-to-noise.)
...
13. **Verification infrastructure is production code.** Hooks, gate scripts, and multi-runtime mirrors get the same discipline they impose on `src/`: each guard ships a co-located test with a positive case (blocks the forbidden) and a negative case (passes the legitimate); a hook that lands on no vector its callers actually take is dead until proven live; a wall that covers only `Write|Edit|MultiEdit` leaves the Bash door open (heredoc, `sed -i`, `tee`, `node -e`) and a coverage number no one measured is a hope, not a guarantee. Detection power is a proven invariant — coverage + diff-coverage in the gate from bootstrap, mutation testing as a ritual past ~1000 cases, property-based checks on pure calculation, golden/approval on complex business output, and mock fidelity tested against the boundary manifest — so "the tests run" becomes "the tests catch". A guarding rule reads in the language its project speaks, and a blocking rule ships in shadow mode before it enforces. **The boundary against over-verification:** deterministic infrastructure (hooks, gates, coverage) and independent adversarial review against a spec (a reviewer reading a diff it did not write) are production code and stay — they run outside the model, or with a different brief. What goes is *prose asking the model to re-inspect its own output*; current models already do it, and the instruction produces self-built verification pipelines that consume the budget before results land (p.26). See `references/hook-canonical-patterns.html` and `templates/rules/testing.md.tpl`.
...
- List of hooks to install (always: session-start-env-check, track-workflow, enforce-workflow, precompact-handoff, postcompact-reinject, bash-write-guard). `bash-write-guard` (PreToolUse `Bash`) closes the Write/Edit blind spot the other guards leave — fail-open, shadow mode by default (`BASH_WRITE_GUARD_ENFORCE=1` to enforce). When the project has a JS test runner, the hook test harness ships alongside: `lib/hook-test-util.mjs` + co-located `*.test.mjs` per guard (positive+negative) + `hooks-inventory.test.mjs` (zero orphan hook).
- List of scripts to install (always: file-size-growth-guard, quality-checks/, setup-worktree, diff-coverage, sample-review, loop-sla, predeploy-check; palier-5: check-runtime-parity + `runtime-parity.json`) and `risk-tiers.json` at `.claude/` root (path tiering — a critical-path diff is never BATCH); check-model-routing + `.claude/model-routing.json` (registre de routage des modèles, gate en mode warn au bootstrap).
- `package.json` script additions (validate, validate:fast, quality:check, size-guard, test, test:coverage, diff-coverage). `bootstrap-project.mjs` merges these deterministically (add-if-missing, package-manager-aware), so non-husky/small projects get them too — not just docs-* scripts. `diff-coverage` runs inside `validate` (before the mark-validate-pass sentinel) as a new-bootstrap default; on MIGRATE it stays opt-in per project so an installed `validate` is never broken. `claude:model-routing:check` est câblé en TÊTE de validate/validate:fast dès le bootstrap : en mode `'warn'` il ne peut pas casser la chaîne, et il devient bloquant par une promotion explicite du registre (`enforcement:'error'`) — un gate bloquant s'installe en observation avant de mordre.
...
2. **Drift** — files past file-size-budget, defensive scaffolding patterns in standing context (see `references/anti-pattern-catalog.html`), markdown-docs drift (`.md` files under `docs/`, missing docs-html toolkit/hook/docs-map/index), versions de modèle figées dans la prose gouvernée (`.claude/rules`, `.claude/skills`, `.claude/agents` — jamais CLAUDE.md ni docs/).
...
5. **Palier** — current evolution stage per `references/evolution-roadmap.html`. Triggers met for next palier? Palier 4 is "release-ready": server CI is an option with a documented local compensation (see `ciPolicy`), not a hard requirement — don't flag its absence as a gap when `ciPolicy` is `local-compensation`.
...
- A new Claude Code model release requires standard updates (the refresh touches THREE surfaces in the same change-set: the CLAUDE.md « Default model + effort » block, `references/model-effort-defaults.html` § Current lineup, and each project's `.claude/model-routing.json` § current)
...
**CI policy is the user's call (palier 4).** Server CI + branch protection are a recommended option, not a blocking invariant. When promoting to palier 4, record the user's decision in `.claude/.meta-govern.json` `ciPolicy` (`'server-ci'` | `'local-compensation'`). `local-compensation` means the local substitute is in place — `predeploy-check.mjs` (clean worktree + green validate before a deploy/sync, user-invoked) plus the hardened Stop gate (`MG_HEADLESS_RUN`) — and AUDIT then presents missing server CI as an option, not a gap.
