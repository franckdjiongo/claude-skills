...
- List of scripts to install (always: file-size-growth-guard, quality-checks/, setup-worktree, diff-coverage, sample-review, loop-sla, predeploy-check; palier-5: check-runtime-parity + `runtime-parity.json`) and `risk-tiers.json` at `.claude/` root (path tiering — a critical-path diff is never BATCH).
- `package.json` script additions (validate, validate:fast, quality:check, size-guard, test, test:coverage, diff-coverage). `bootstrap-project.mjs` merges these deterministically (add-if-missing, package-manager-aware), so non-husky/small projects get them too — not just docs-* scripts. `diff-coverage` runs inside `validate` (before the mark-validate-pass sentinel) as a new-bootstrap default; on MIGRATE it stays opt-in per project so an installed `validate` is never broken.
...
- A new Claude Code model release requires standard updates (e.g., Opus 4.8 → Opus 5 prompt rewrites)
