...
- When the caller confirms it holds the canon templates in context (or can read them), reference each delta by anchor + template path + line range rather than reproducing the full canon text — reproducing text the caller already has inflates the plan (135–385k tokens observed this way). Reproduce full text only for renderings that DIFFER from canon (localized or translated variants), where the exact bytes are load-bearing.
